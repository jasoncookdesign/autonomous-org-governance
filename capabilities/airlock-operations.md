...
| Sandbox inbox folder | Designated filesystem path on the sandbox host | Inbound only | CEO places file; Operations Director retrieves |
| Email | From [YOUR_PERSONAL_EMAIL] to [YOUR_ORG_EMAIL] | Inbound only | Operations Director monitors [YOUR_ORG_EMAIL] inbox |
...
- Sandbox inbox filesystem folder
- [YOUR_ORG_EMAIL] email inbox (monitoring for CEO-sourced transfers)
...
- CEO personal email ([YOUR_PERSONAL_EMAIL]) — inbound detection only via org email inbox
