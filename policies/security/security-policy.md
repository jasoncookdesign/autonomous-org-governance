...
- Emails (except from CEO at [YOUR_PERSONAL_EMAIL] via approved airlock channel)
...
| Google Drive (via [YOUR_ORG_EMAIL]) | Core approved |
| Gmail (via [YOUR_ORG_EMAIL]) | Core approved |
...
**Google Workspace scope limitation:** Only Google Drive and Gmail (via [YOUR_ORG_EMAIL]) are approved. All other Google Workspace tools -- including but not limited to Google Docs, Google Sheets, Google Slides, Google Calendar, Google Meet, Google Forms, Google Sites, and Google Chat -- are unapproved until explicitly requested and CEO-approved. The fact that these tools are accessible within the same authenticated Google session does not constitute approval to use them.
...
- Sandbox accounts use the [YOUR_ORG_EMAIL] identity or purpose-built organizational accounts
- No new external accounts may be created without CEO approval

## Network Security

- The sandbox host uses a fixed DHCP reservation: `[YOUR_SANDBOX_HOSTNAME]` / `[YOUR_SANDBOX_IP]`
...
| CEO reports and escalations | `capabilities/president-coordination.md` | [YOUR_PERSONAL_EMAIL] | Session digests, weekly summaries, escalation requests, incident notifications |
