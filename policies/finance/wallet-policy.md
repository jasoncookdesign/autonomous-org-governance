...
All organizational wallets are provisioned through **[YOUR_VIRTUAL_CARD_PROVIDER]** as virtual credit cards. Wallets may not exist on day one; the organization must request wallet provisioning from the CEO when a wallet is needed and unavailable.
...
3. The CEO provisions the wallet via [YOUR_VIRTUAL_CARD_PROVIDER] and communicates the wallet parameters back to the organization
...
Inactive directors (Research, Financial Analysis, Creative) do not have wallets until activated.
...
| Parameter | Value |
| --- | --- |
| Transaction limit | [YOUR_TX_LIMIT]/transaction — set per wallet at provisioning |
| Monthly cap | [YOUR_MONTHLY_CAP]/month — set per wallet at provisioning |
| Maximum acceptable loss | [YOUR_MAL] Maximum Acceptable Loss — set per wallet at provisioning |
