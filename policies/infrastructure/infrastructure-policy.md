...
The sandbox organization operates on a dedicated [YOUR_SANDBOX_HOST]. This machine is the primary compute, storage, and execution environment for all sandbox agents and tools.

The sandbox host is a single-tenant environment. It is not shared with personal workloads unless the CEO explicitly approves a cross-use case.

## Network Identity

| Parameter | Value |
| --- | --- |
| Hostname | `[YOUR_SANDBOX_HOSTNAME]` |
| IP Address | `[YOUR_SANDBOX_IP]` (DHCP reservation on router) |
| VPN | Tailscale (approved for secure remote access — replaceable with any equivalent VPN tool) |
...
| Account Type | Rule |
| --- | --- |
| CEO personal accounts | Must not be signed in on the sandbox host (except as explicitly noted below) |
...
| Organizational accounts | [YOUR_ORG_EMAIL] and purpose-built organizational accounts only |
...
| Containers | Docker |
| Network | Tailscale |
| Communication | Google Drive (via [YOUR_ORG_EMAIL]) -- approved |
| | Gmail (via [YOUR_ORG_EMAIL]) -- approved |
...
**Google Workspace scope:** Only Google Drive and Gmail (via [YOUR_ORG_EMAIL]) are approved at launch. Accessibility of other Google tools within the same authenticated session does not constitute approval. Google Docs, Sheets, Slides, Calendar, Meet, Forms, Sites, Chat, and all other Google Workspace products require CEO approval before use.

## Backup

Backup is handled at the operating system level and is not the sandbox organization's direct responsibility. The organization must not create independent backup mechanisms that copy KB content or policy documents to unapproved locations.
...
The sandbox host is a dedicated [YOUR_SANDBOX_HOST] with [YOUR_CPU_GPU_SPECS], [YOUR_RAM] unified memory, and [YOUR_STORAGE] internal storage. [YOUR_EXTERNAL_STORAGE] external storage is attached for document storage and backup.

**Concurrent agent processes:** Maximum [YOUR_MAX_AGENTS] concurrent agent processes at any time. Additional process requests must queue or be deferred until a slot is available. The President Agent is always counted against this limit when active.

**Per-agent resource limits:**

| Resource | Limit per agent |
| --- | --- |
| Memory | [YOUR_RAM_PER_AGENT] RAM per agent |
| CPU cores | [YOUR_CPU_PER_AGENT] CPU cores per agent |

**Host-level ceiling:** Total agent memory must not exceed [YOUR_TOTAL_RAM_CEILING] total. If the ceiling would be breached by a new process, the requesting agent must wait or escalate to the CEO.

**GPU access:** GPU-intensive workloads (e.g., local model inference) require CEO approval before execution. No GPU-intensive workloads are approved at launch.
