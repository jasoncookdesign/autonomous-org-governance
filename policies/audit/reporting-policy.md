...
All reports are delivered by email from [YOUR_ORG_EMAIL] to [YOUR_PERSONAL_EMAIL]. Reports must be clearly labeled with type and date in the subject line. No report may be sent to any external party.
...
**Delivered by:** President Agent (via [YOUR_ORG_EMAIL])
...
2. President Agent delivers via email ([YOUR_ORG_EMAIL] to [YOUR_PERSONAL_EMAIL])
